...
def zd(mese, dia):
    ...
    zd_days=((1,20),(2,19),(3,21),(4,20),(5,21),(6,21),
                (7,22),(8,23),(9,23),(10,23),(11,22),(12,22))     #元组嵌套
    month = mese
    day = dia
...
def world_c(year, month, day): 
    ...
    sj = int(str(y)[0:2]) + 1
    s = int(str(y)[2:4])
    m_j = (30 * (((-1)**m1 + 1)/2)) + int((3*m1-7)/5)  # 月基数
    x = (44 * (sj-1) + int((sj-1)/4) + 9) % 60  # 世纪常数
    s1 = int(s/4)
    u = s % 4
    r = (s1*6 + 5*(s1*3 + u) + m_j + d + x) % 60  # 高氏日柱公式
    return r
...
num_all = {int(x1), int(x2), int(x3), int(x4), int(x5), int(x6), int(x7), int(x8),int(t1), int(t2), int(l), int(s)}
...
nu1 = sum(item["nu1"] for item in new) * 1.8094767
nu2 = sum(item["nu2"] for item in new) * 0.8312839
nu3 = sum(item["nu3"] for item in new) * 0.6929213
nu4 = sum(item["nu4"] for item in new) * 0.9119807
nu5 = sum(item["nu5"] for item in new) * 1.4897836
nu6 = sum(item["nu6"] for item in new) * 0.9676069
f = sum(item["f"] for item in new)
m = sum(item["m"] for item in new)

f_day = [n, p, nu1, nu2, nu3, nu4, nu5, nu6, f, m]
# print(f_day)